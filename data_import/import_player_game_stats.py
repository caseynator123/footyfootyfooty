...
def get_data(year, team):
    ...
    try:
        ...
        for table in stat_tables:
            ...
            player_stats = pd.melt(player_stats, id_vars=['Player']).rename(columns={'variable':'round','value':desc})
        ...
        opp_played = df[df['Player'] == 'Opponent'].melt(id_vars = 'Player').drop(columns='Player').rename(columns={'variable':'round','value':'opponent'})
...
for team in teams_str:
    for year in range(2023,2024):
        ...
        main.to_csv(f'/Users/caseyh/Desktop/footyfootyfooty/player_stats/{year}_player_stats.csv', index=False)